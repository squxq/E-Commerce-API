...
class Variation {
    ...
    `;
        ...
        if (checkCategoryQuery[0].names.find((existingName) => {
    ...
    checkValues(value, values) {
        if (!value && !values)
            throw new ApiError(httpStatus.BAD_REQUEST, "At least one value is required!");
    ...
    `;
        if (result.length === 0)
            throw new ApiError(httpStatus.NOT_FOUND, `Invalid variation: ${variationId} provided!`);
        this.values.sort();
    ...
    async getSKUInfo(variationOptions = null, variationId = null, multiple = false, productItemIds = null) {
      ...
      WHERE a.id IN (${productItemIds ? Prisma.join(productItemIds) : Prisma.sql `SELECT product_item_id FROM product_item_info`})
      GROUP BY a.id, "SKU"
    `;
    }
    ...
    `;
        if (variation.length === 0)
            throw new ApiError(httpStatus.NOT_FOUND, `Variation ${variationId} not found!`);
    ...
    `;
      ...
      `;
            return Object.assign(Object.assign({}, (await this.deleteVariation(variationOptionIds))), { productItems: productItemsArr, productConfigurations: deleteProductConfigurations });
        }
        ...
        `;
            ...
            return Object.assign(Object.assign({}, this.deleteVariation(variationOptionIds)), { productItems: productItemsArr, incompatibilities: deleteProductTree });
        }
        throw new ApiError(httpStatus.BAD_REQUEST, `Products cant be saved! Change: save=false or add another variation option to the following products: ${productItemIds}.`);
    }
    ...
    `;
        if (option.length === 0)
            throw new ApiError(httpStatus.NOT_FOUND, `Variation option: ${optionId} not found!`);
