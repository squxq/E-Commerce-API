...
class Category {
    ...
    async validateName() {
      ...
      `;
            if (result.length === 0)
                throw new ApiError(httpStatus.NOT_FOUND, `Parent category: ${this.parentId} not found!`);
            if (!result[0].names[0])
      ...
      `;
        ...
        if (result[0].names.find((name) => {
            return formatName(name) === formatName(this.categoryName);
        })) {
            throw new ApiError(httpStatus.BAD_REQUEST, `Duplicate category name provided! ${this.categoryName} is already in use.`);
        }
        return this.categoryName;
    }
    // update SKUs
    async updateSKUs(categoryId) {
      ...
      `;
            const productImages = Array.from(new Map(products.map((obj) => [obj.image, { id: obj.id, images: obj.image }])).values());
            ...
            const deleteImages = Array.from(new Set([...productImagesPublicIds, [].concat(...productItemImages.map((obj) => obj.images))])).map(async (image) => deleteImage(image));
            await Promise.all(deleteImages);
            // delete folders from cloudinary
            const folders = Array.from(new Set(productImagesPublicIds.map((image) => image.substring(0, image.lastIndexOf("/"))))).map(async (folder) => deleteFolder(folder));
      ...
      `;
        ...
        UPDATE product SET
          image = v.newImage
        FROM (VALUES
          ${Prisma.join(mainImagesArray.map((row) => {
                return Prisma.sql `(${Prisma.join(row)})`;
            }))}
    ...
    async validateParent(categoryId, categoryName) {
      ...
      `;
        if (category.length === 0)
            throw new ApiError(httpStatus.NOT_FOUND, `Category: ${categoryId} not found!`);
   ...
   `;
    ...
    async updateResources(reverse = false, childId = null, newParentId = null) {
        const updateResourcesTransaction = await prismaInbound
            ...
            if (err.meta.code === "23505") {
                throw new ApiError(httpStatus.BAD_REQUEST, `Resources cant be saved. Change: save=false or change resources data: ${err.meta.message}.`);
            }
    ...
    async deleteTree(lastLayer, categoryId = null) {
      ...
      `;
        ...
        `;
                // delete images
                const imagesArr = Array.from(new Set([...products.map(({ image }) => image)].concat(...productItems.map(({ images }) => images)))).map(async (image) => {
                    return deleteImage(image);
                });
                await Promise.all(imagesArr);
                // delete folders
                const folderPromise = Array.from(new Set(products.map(({ image }) => image.substring(0, image.lastIndexOf("/"))))).map(async (folder) => {
    ...
    async deleteValidation(categoryId) {
        ...
        // check for parent id
        if (!this.parentId) {
            throw new ApiError(httpStatus.BAD_REQUEST, "Its not possible to save resources deleting the current category. Change: save=false in order to delete the category.");
        }
        if (resources.length > 0) {
            // check for siblings
