...
class CreateProductItem {
    ...
    // checkOptions
    async checkOptions() {
        const allowedOptions = await prismaInbound.$queryRaw `
            ...
            const exists = allowedOptions.find((option) => {
                if (option.name === key) {
                    if (!option.values.includes(value)) {
                        throw new ApiError(httpStatus.BAD_REQUEST, `Wrong variation option provided for: ${key}, value: ${value} does not exist!`);
                    }
                }
                return option.name === key;
            });
            if (!exists)
                throw new ApiError(httpStatus.BAD_REQUEST, `Variation ${key} does not exist!`);
        });
    ...
    async checkPrice() {
        ...
        let formattedPrice = Math.floor(parseFloat(this.price.value) * result.currency.base[result.currency.base.length - 1] ** result.currency.exponent);
    ...
    generateSKU(categoryName, productName, brandName, options) {
        ...
        sku = sku.concat(Object.values(orderedOptions).map((option) => {
    ...
    async createProductConfigurations(prisma, variationsIds, productItemId) {
      ...
      VALUES ${Prisma.join(valuesParams.map((row) => {
    ...
    async getProductName(productId) {
        const product = await prismaInbound.product.findUnique({
            where: { id: productId },
            select: { name: true },
        });
        return product.name;
    ...
    async uploadImages(images, productName = null, productId = null, buffer = null, main = false, categoryName = null) {
        if (!productName) {
            if (productId) {
                ...
                throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, "Its not possible to upload images without a product name.", false);
            }
        }
    ...
    async validateImage(image, productId, productName) {
        ...
        let images = await prismaInbound.$queryRaw `
      SELECT * FROM (
        (
    ...
    `;
        ...
        if (!publicId.substring(publicId.lastIndexOf("/") + 1).startsWith("main")) {
            ...
            const newPublicId = await updateName(publicId, `${publicId.substring(0, publicId.lastIndexOf("/") + 1)}main_${publicId.substring(publicId.lastIndexOf("/") + 1)}`);
    ...
    async deleteProductItems(prisma, products, multiple = false) {
        ...
        RETURNING id, product_item_id, variation_option_id
      `;
            const imagesPromises = Array.from(new Set([
    ...
    `;
      ...
      `;
        ...
        await Promise.all(countResults.map(async ({ publicId, count }) => {
            if (count === 1) {
    ...
    async updateProductItemTransaction(productItemId, data, variationsIds = null) {
        const updateTransaction = prismaInbound.$transaction(async (prisma) => {
        ...
        `;
            ...
            return {
                ...
                [productConfigurations.length === 1 ? "productConfiguration" : "productConfigurations"]: productConfigurations.length !== 0 ? productConfigurations : undefined,
            };
        });
    ...
    `;
        ...
        const validProducts = productVariations.map(({ product_item_id: id, variation_names: names, variation_values: values }) => {
            ...
            if (save && variations.length !== names.length) {
                throw new ApiError(httpStatus.BAD_REQUEST, `Its not possible to save all resources due to the new product category not having the variations: ${names.join(", ")} with the variation options: ${values.join(", ")}`);
            }
            return Object.assign({ id, delete: variations.length === 0 }, (variations.length > 0 && { variations }));
        });
        ...
        if (!validProducts.find(({ delete: toDelete }) => toDelete === false)) {
            throw new ApiError(httpStatus.BAD_REQUEST, "Product cannot be updated to the specified category because no product item can be saved");
        }
        ...
        if (Array.from(new Set(options)).length !== options.length) {
            throw new ApiError(httpStatus.BAD_REQUEST, `Result from deleting variations such that product items can be saved, is that some product_items have the same variation options, which is not allowed`);
        }
            ...
            if (!toDelete) {
              ...
              `;
                    if (newVariation.length === 0)
                        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, "Something went wrong", false);
    ...
    async deleteProductTransaction(productId) {
        const deleteNewProductTransaction = await prismaInbound.$transaction(async (prisma) => {
            // validate product id
      ...
      `;
            ...
            return Object.assign({ product: deletedProduct }, productItems);
        });
    ...
    async checkImages(images, productId) {
        ...
        const imageArray = await prismaInbound.$queryRaw `
      SELECT * FROM (
        SELECT image
        FROM product
        WHERE id = ${productId}
    ...
    `;
        ...
        const existingImages = newImageArray.filter((image) => providedImages.indexOf(image.substring(image.length - 32)) !== -1);
        ...
        const toUploadImages = images.filter((image) => toUploadHashArray.indexOf(hash(parser(image).content, { algorithm: "md5" })) !== -1);
        return {
    ...
    `;
      ...
      UPDATE product_item SET
        images = v.imagesArr
      FROM (VALUES
        ${Prisma.join(newImagesArray.map((row) => {
            return Prisma.sql `(${Prisma.join(row)})`;
        }))}
...
const createProduct = catchAsync(async (data, images) => {
    ...
    // check name
    const names = await prismaInbound.$queryRaw `
    SELECT name AS name
    FROM product
    ...
    const createProductTransaction = await prismaInbound.$transaction(async (prisma) => {
        const createNewProductTransaction = await prisma.product
            ...
            if (err.code === "P2002" && ["category_id", "name"].every((element) => err.meta.target.includes(element))) {
                throw new ApiError(httpStatus.BAD_REQUEST, `Duplicate product name provided! ${createNewProductTransaction.name} is already in use.`);
            }
            throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, err.message, false);
        });
        const [createProductItem, createProductConfiguration] = await createNewProduct.createProductItemTransaction(createNewProductTransaction.id, sku, orderedOptions, imagesArray, formattedPrice, prisma);
        return {
...
const createProductItem = catchAsync(async (productId, quantity, price, options, images) => {
    ...
    const createProductItemTransaction = await prismaInbound.$transaction(async (prisma) => {
        const [createNewProditem, createProductConfiguration] = await createNewProductItem.createProductItemTransaction(productId, sku, orderedOptions, imagesArray, formattedPrice, prisma);
        return {
...
const updateProductItem = catchAsync(async (data, images, query) => {
  ...
  `;
    if (productItem.length === 0)
        throw new ApiError(httpStatus.NOT_FOUND, `Product Item: ${productItemId} not found!`);
    ...
    if (data.options) {
        ...
        // re-generate sku
        const { sku, orderedOptions } = updateNewProductItem.generateSKU(productItem[0].category_name, productItem[0].product_name, productItem[0].product_brand);
        // eslint-disable-next-line no-param-reassign
        data.SKU = sku.join("-");
        variationsIds = await updateNewProductItem.verifyProductItemOptions(prismaInbound, orderedOptions, productItem[0].product_id);
        // eslint-disable-next-line no-param-reassign
        delete data.options;
    }
    if (images.length > 0) {
        const { toUpload, existing } = await updateNewProductItem.checkImages(images, productItem[0].product_id);
        let imagesArray = await updateNewProductItem.uploadImages(toUpload, productItem[0].product_name, null, null, false, productItem[0].category_name);
        imagesArray.pop();
        imagesArray = [...existing, ...imagesArray];
        if (query.images === "add") {
    ...
    const updateProductItemTransaction = await updateNewProductItem.updateProductItemTransaction(productItemId, data, variationsIds);
    return Object.assign({}, updateProductItemTransaction);
});
/**
...
const deleteProductItem = catchAsync(async (productItemId, save) => {
    ...
    const validateProductItem = await prismaInbound.$queryRaw `
    ...
    if (!validateProductItem[0].other_items) {
        if (save) {
            throw new ApiError(httpStatus.BAD_REQUEST, "Its not possible to save the product, due to the fact that this is the only product item left. Change: save=false or add another product item to this product.");
        }
        // delete the product itself
        return deleteNewProductItem.deleteProductTransaction(validateProductItem[0].product_id);
    }
    const deleteProductItemTransaction = await prismaInbound.$transaction(async (prisma) => {
...
//# sourceMappingURL=inbound.product.service.js.map