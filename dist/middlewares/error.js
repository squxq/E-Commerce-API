...
const errorConverter = (err, req, res, next) => {
    ...
    if (!(error instanceof ApiError)) {
        const statusCode = error.statusCode || error instanceof mongoose.Error || error instanceof Prisma.PrismaClientKnownRequestError
...
const errorHandler = (err, req, res, next) => {
    ...
    const response = Object.assign(Object.assign({ type: "Error", code: statusCode, message }, (config.env === "development" && { isOperational: err.isOperational })), (config.env === "development" && { stack: err.stack.replace(/"/g, "'") }));
